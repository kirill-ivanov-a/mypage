...
def contacts_page():
    ...
    return render_template("contacts.html", vkuser=session.get('vkuser'), question_form=question_form)


def create_question(question_form):
    vkuser = session.get('vkuser')
    if vkuser:
        question = Question(vkuser.get('id'), question_form.category.data,
                            question_form.question.data, question_form.detailed_description.data)
        return question
...
def questions_page():
    ...
    return render_template("questions.html", answers=answers, category=category, categories=CategoryEnum)
